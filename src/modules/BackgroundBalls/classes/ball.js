function Ball(options) {
    ...
    this.move = (xMove, yMove) => {
        ...
        // this.opt.speed
        if (this.opt.pos.x + velocity.x <= this.opt.radius || this.opt.pos.x + velocity.x > this.opt.ctx.canvas.width - this.opt.radius) {
            velocity.x *= -1;
        }

        if (this.opt.pos.y + velocity.y <= this.opt.radius || this.opt.pos.y + velocity.y > this.opt.ctx.canvas.height - this.opt.radius) {
            velocity.y *= -1;
        }
